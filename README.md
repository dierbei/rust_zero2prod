...
```
```text
4. User Stories Are Not Set In Stone

这句话的意思是，在敏捷开发过程中，用户故事并不是一个固定不变的清单，它们可以根据客户的需求和反馈进行修改或添加。

这也是敏捷开发方法的一个核心理念，即在不断迭代和反馈的过程中，不断完善和调整产品的需求和功能，以提高客户的满意度和产品的质量。

因此，在敏捷开发过程中，用户故事应该被视为一个动态的需求表达方式，而不是一个固定不变的清单。
```